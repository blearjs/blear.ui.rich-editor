...
var getDataTransferItems = function (dataTransfer) {
    ...
    if (dataTransfer) {
        ...
        var dataItems = dataTransfer.items;
        if (dataItems) {
            for (var i = 0; i < dataItems.length; i++) {
                var dataItem = dataItems[i];
                var kind = dataItem.kind;
                var contentType = dataItem.type;

                if (kind === 'file') {
                    items[contentType] = dataTransfer.files[i];
                } else {
                    try {
...
var pasteImageData = function (editor, e, rng) {
        ...
        if (image) {
            ...
            editor.settings.imagePasteHandler(image, function (err, meta) {
                ...
                editor.undoManager.transact(function () {
                    var imgHTML = '<img';
                    global$3.each(meta, function (val, key) {
                        imgHTML += ' ' + key + '="' + val + '"';
                    });
                    imgHTML += '>';
                    pasteHtml$1(editor, imgHTML, false);
                });
            });
        }
    ...
    if (dataTransfer) {
        return processItems(dataTransfer.files);
    }
};
